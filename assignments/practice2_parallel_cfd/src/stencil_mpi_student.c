...
static void exchange_halos(double *u,int myrows,int n,int rank,int size){
    ...
    /* TODO MPI #1
       Exchange the first owned row (local row 1) with the upper neighbour.
       The row received from the lower neighbour belongs in the lower halo
       (local row myrows+1). Use tag 10.

       Add one MPI_Sendrecv(...) call here.
    */

    /* TODO MPI #2
       Exchange the last owned row (local row myrows) with the lower neighbour.
       The row received from the upper neighbour belongs in the upper halo
       (local row 0). Use tag 20.
...
int main(int argc,char **argv){
    ...
    if(rank==0){
        printf("MODE=MPI\nGRID=%dx%d\nSTEPS=%d\nRANKS=%d\n",n,n,steps,size);
        printf("ROW_COUNTS=[");
        for(int r=0;r<size;r++){ int c=base+(r<extra?1:0); printf("%s%d",r?",":"",c); }
        printf("]\nROW_DISPLS=[");
        for(int r=0;r<size;r++){ int d=r*base+(r<extra?r:extra); printf("%s%d",r?",":"",d); }
        printf("]\n");
    }
    printf("rank %d: rows=%d-%d count=%d host=%s\n",rank,start,end,myrows,host);
    fflush(stdout);
    ...
    if(center>=start && center<=end)
        center_value=u[(size_t)(center-start+1)*n+center];
