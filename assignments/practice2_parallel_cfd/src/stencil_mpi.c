...
int main(int argc,char **argv){
    ...
    int center=n/2;
    if(center>=start && center<=end){ int li=(center-start)+1; u[(size_t)li*n+center]=100.0; }

    if(rank==0){
        printf("MODE=MPI\nGRID=%dx%d\nSTEPS=%d\nRANKS=%d\n",n,n,steps,size);
        printf("ROW_COUNTS=[");
        for(int r=0;r<size;r++){ int c=base+(r<extra?1:0); printf("%s%d",r?",":"",c);} printf("]\n");
        printf("ROW_DISPLS=[");
        for(int r=0;r<size;r++){ int d=r*base+(r<extra?r:extra); printf("%s%d",r?",":"",d);} printf("]\n");
    }
    ...
    for(int s=0;s<steps;s++){
        ...
        MPI_Sendrecv(&u[1*n],n,MPI_DOUBLE,up,10,
                     &u[(myrows+1)*n],n,MPI_DOUBLE,down,10,
                     MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        MPI_Sendrecv(&u[myrows*n],n,MPI_DOUBLE,down,20,
