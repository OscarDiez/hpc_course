...
cat << 'MODULE_EOF' > \$MODULEFILES_DIR/openmpi
#%Module1.0
set root /usr/lib/openmpi
prepend-path PATH \$root/bin
prepend-path LD_LIBRARY_PATH \$root/lib
MODULE_EOF

# Create a modulefile for MPICH
cat << 'MODULE_EOF' > \$MODULEFILES_DIR/mpich
#%Module1.0
set root /usr/lib/mpich
prepend-path PATH \$root/bin
prepend-path LD_LIBRARY_PATH \$root/lib
MODULE_EOF

# Create a modulefile for GROMACS
cat << 'MODULE_EOF' > \$MODULEFILES_DIR/gromacs
#%Module1.0
...
echo "Modulefiles created for OpenMPI, MPICH, and GROMACS."
