...
def _():
    ...
    def compute_square_1(num):
        return num * num
    if __name__ == '__main__':
        numbers_1 = list(range(10))
        with ProcessPoolExecutor(max_workers=4) as executor:
            squares_1 = list(executor.map(compute_square_1, numbers_1))
...
def _(Process_1):
    ...
    if __name__ == '__main__':
        ...
        lock = Lock()
        processes = [Process_1(target=increment, args=(shared_value, lock)) for _ in range(4)]
        for p in processes:
            p.start()
        for p in processes:
            p.join()
