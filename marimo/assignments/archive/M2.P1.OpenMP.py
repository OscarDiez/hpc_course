...
def _(os, subprocess):
    ...
    if os.path.exists(_c_program_path):
        os.remove(_c_program_path)
    _c_program = '\n#include <omp.h>\n#include <stdio.h>\n#include <stdlib.h>\n#include <time.h>\n\n#define NUM_SIMULATIONS 1000000\n#define MAX_THREADS 8\n\ndouble monte_carlo_simulation(int seed) {\n    srand(seed);\n    double x = (double)rand() / RAND_MAX;\n    double y = (double)rand() / RAND_MAX;\n    return x * y;\n}\n\nint main() {\n    double results[NUM_SIMULATIONS];\n    double sum = 0.0;\n\n    for (int num_threads = 1; num_threads <= MAX_THREADS; num_threads++) {\n        sum = 0.0;\n        double start_time = omp_get_wtime();\n\n        #pragma omp parallel for num_threads(num_threads) reduction(+:sum)\n        for (int i = 0; i < NUM_SIMULATIONS; i++) {\n            results[i] = monte_carlo_simulation(i);\n            sum += results[i];\n        }\n\n        double average = sum / NUM_SIMULATIONS;\n        double end_time = omp_get_wtime();\n\n        printf("Threads: %d, Average Result: %f, Execution Time: %f seconds\\n", num_threads, average, end_time - start_time);\n    }\n\n    return 0;\n}\n'
    with open(_c_program_path, 'w') as _file:
        _file.write(_c_program)
    print(f'C program written to {_c_program_path}.')
    _compile_command = f'gcc -o monte_carlo_openmp -fopenmp {_c_program_path}'
    _compile_process = subprocess.run(_compile_command, shell=True, capture_output=True, text=True)
    print('Compiling the C program...')
    if _compile_process.stdout:
        print('Output:', _compile_process.stdout)
...
def _(mo):
    mo.md(
        ...
        Monte Carlo simulations are used to estimate the probability of various outcomes by performing repeated random sampling. The code provided demonstrates how to parallelize this simulation using OpenMP.

        - **Parallelization:** We use the `#pragma omp parallel for` directive with `reduction(+:sum)` to allow multiple threads to perform the simulation in parallel. The reduction clause ensures that the results from each thread are correctly accumulated into the `sum` variable.

        - **Thread Scaling:** The program runs the simulation with 1 to 8 threads, comparing the execution time and average results. This allows us to observe the impact of parallelization on performance.

        - **Time Measurement:** The execution time is measured for each number of threads, showing how the workload is distributed and how it affects the overall time to complete the simulations.

        By comparing the execution time across different numbers of threads, you can see the benefits of parallel computing in reducing the time required for large-scale simulations.
