...
app._unparsable_cell(
    ...
    int main(int argc, char *argv[]) {
        ...
        for (int i = 0; i < 100000; i++) {
            ...
            if (i % 20000 == 0) {
                printf(\"Process %d doing computation during reduction...\\n\", rank);
            }
        }
    ...
    # Set environment variables to allow running as root (necessary for Colab)
    os.environ['OMPI_ALLOW_RUN_AS_ROOT'] = '1'
    os.environ['OMPI_ALLOW_RUN_AS_ROOT_CONFIRM'] = '1'
...
def _(mo):
    mo.md(
        r"""
        ## Exercise: Exploring Non-Blocking Communication in MPI

        In the previous example, we used non-blocking communication with `MPI_Ireduce` to allow the processes to continue their computation while the reduction was in progress. However, all processes are performing the same dummy computation.

        ### Task:

        1. **Modify the Code**: Change the code so that each process performs a different amount of computation during the reduction phase. For example:
           - Process 0 does 5 loops.
           - Process 1 does 20 loops.
           - Process 2 does 50 loops.
           - Process 3 does 100 loops.

        2. **Observe the Behavior**: After modifying the loop conditions, run the code again and observe how the different workloads affect the timing of when each process completes its computation and reaches the `MPI_Wait` call.

        3. **Reflection**: Write a brief explanation on how non-blocking communication benefits parallel programs, particularly when processes have imbalanced workloads.
...
def _(mo):
    mo.md(
        r"""
        ## Exercise: Understanding Workload Imbalance with Non-Blocking MPI Communication

        In the previous example, all processes performed the same amount of computation during the non-blocking reduction. However, in real-world scenarios, processes may have different workloads, leading to imbalanced computations.

        ### Task:

        1. **Modify the Code**: Change the code so that each process performs a different amount of computation during the reduction phase. Specifically:
           - Process 0 does 5 loops.
           - Process 1 does 20 loops.
           - Process 2 does 50 loops.
           - Process 3 does 100 loops.

        2. **Run the Modified Code**: Run the modified code and observe how each process performs different amounts of computation while the non-blocking reduction is taking place.

        3. **Reflection**: After running the code, write a short explanation on how non-blocking communication helps handle imbalanced workloads in parallel computing.

        ### Hint:

        You need to modify the loop that simulates the computation for each process and use a `switch` or `if-else` structure to assign different numbers of iterations to different processes.
...
app._unparsable_cell(
    r"""
    # Writing the MPI non-blocking collective operation with different workloads to a file
    mpi_code_solution = \"\"\"
    #include <mpi.h>
    #include <stdio.h>
    ...
    int main(int argc, char *argv[]) {
        ...
        // Each process performs a different amount of computation
        int loop_count;
        switch (rank) {
            case 0: loop_count = 5; break;
            case 1: loop_count = 20; break;
            case 2: loop_count = 50; break;
            case 3: loop_count = 100; break;
        }
        ...
        for (int i = 0; i < loop_count; i++) {
            // Perform some dummy computation
            if (i % (loop_count / 5) == 0) {
                printf(\"Process %d doing computation during reduction... (step %d)\\n\", rank, i);
            }
        }
    ...
    # Save the C program to a file
    with open(\"mpi_nonblocking_collective_solution.c\", \"w\") as mpi_file:
        mpi_file.write(mpi_code_solution)

    # Compile the C program using mpicc
    !mpicc -o mpi_nonblocking_collective_solution mpi_nonblocking_collective_solution.c

    # Set environment variables to allow running as root (necessary for Colab)
    os.environ['OMPI_ALLOW_RUN_AS_ROOT'] = '1'
    os.environ['OMPI_ALLOW_RUN_AS_ROOT_CONFIRM'] = '1'

    # Run the MPI program with 4 processes, demonstrating non-blocking reduction with different workloads
    !mpirun --allow-run-as-root --oversubscribe -np 4 ./mpi_nonblocking_collective_solution
    """,
    name="_"
)
