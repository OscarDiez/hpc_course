...
def _(mo):
    mo.md(
        ...
        ## Time Complexity Comparison Code

        This code compares the performance of several algorithms with different time complexities. The algorithms are tested on different input sizes, and the time taken for each algorithm to execute is recorded. Below is an explanation of each complexity and the results observed.

        ### 1. O(1) - Constant Time

        - **Description**: The constant time function (`constant_time_operation`) always returns a constant value (42). This operation takes the same amount of time regardless of the input size.
        - **Result**:
            - The time taken remains constant, even as the input size increases. This is expected because the function does not depend on the size of the input.

        ### 2. O(log n) - Binary Search

        - **Description**: Binary search divides the input array in half during each iteration, resulting in logarithmic time complexity. For each input size, the algorithm searches for a random target in a sorted array.
        - **Result**:
            - The time increases slightly with the input size, but the growth is logarithmic, meaning it grows very slowly even as the input size increases significantly.

        ### 3. O(n) - Summing an Array

        - **Description**: Summing an array is a linear operation, where each element of the array is processed exactly once. The time taken increases proportionally with the input size.
        - **Result**:
            - The time taken grows linearly with the size of the array. For instance, doubling the input size roughly doubles the time taken to compute the sum.

        ### 4. O(n log n) - Merge Sort

        - **Description**: Merge sort divides the array into smaller subarrays, sorts them, and then merges them back together. This results in a time complexity of O(n log n).
        - **Result**:
            - The time taken increases with the input size but at a slower rate compared to O(n²) operations. For larger input sizes, merge sort is efficient compared to quadratic algorithms.

        ### 5. O(n²) - Quadratic Operation

        - **Description**: This function (`quadratic_operation`) compares each element of the array with every other element, leading to quadratic time complexity.
        - **Result**:
            - The time increases significantly with the input size, and quadratic operations become inefficient for large inputs. This is why the input size is limited to 1000 in the code.

        ### 6. O(2ⁿ) - Fibonacci (Exponential Time)

        - **Description**: The Fibonacci function is implemented using a naive recursive approach, resulting in exponential time complexity. Each Fibonacci number requires the computation of two smaller Fibonacci numbers, which leads to a large number of redundant calculations.
        - **Result**:
            - The time taken increases exponentially with the input size. Even for small input sizes like 20 or 30, the time taken becomes very large.
...
app._unparsable_cell(
    ...
    // Example data size
    #define N 2000 // Increased N to make time differences noticeable
    ...
    void initialize_matrices(double **A, double **B, double **C, int n) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                A[i][j] = (double)(rand() % 1000) / 1000.0;
                B[i][j] = (double)(rand() % 1000) / 1000.0;
                C[i][j] = 0.0;
            }
        }
    }
    ...
    // Regular O(n) loop
    void regular_loop(double A[], double B[], double C[], int n) {
        for (int i = 0; i < n; i++) {
            C[i] = A[i] + B[i];
        }
    }
    ...
    void matrix_multiplication_tiling(double **A, double **B, double **C, int n, int blockSize) {
    ...
    int main() {
        ...
        regular_loop(A, B, C, N);
        double end = get_time();
        printf(\"Regular Loop Time: %.6f seconds\\n\", end - start);
        ...
        int matrix_size = 500; // Adjust matrix size as needed

        // Allocate matrices
        double **M = (double **)malloc(matrix_size * sizeof(double *));
        double **N1 = (double **)malloc(matrix_size * sizeof(double *));
        double **N2 = (double **)malloc(matrix_size * sizeof(double *));
        double **Result1 = (double **)malloc(matrix_size * sizeof(double *));
        double **Result2 = (double **)malloc(matrix_size * sizeof(double *));
        for (int i = 0; i < matrix_size; i++) {
            M[i] = (double *)malloc(matrix_size * sizeof(double));
            N1[i] = (double *)malloc(matrix_size * sizeof(double));
            N2[i] = (double *)malloc(matrix_size * sizeof(double));
            Result1[i] = (double *)malloc(matrix_size * sizeof(double));
            Result2[i] = (double *)malloc(matrix_size * sizeof(double));
        }

        // Initialize matrices
        initialize_matrices(M, N1, Result1, matrix_size);
        initialize_matrices(M, N2, Result2, matrix_size);

        // Timing naive matrix multiplication
        start = get_time();
        matrix_multiplication(M, N1, Result1, matrix_size);
        end = get_time();
        printf(\"Naive Matrix Multiplication Time: %.6f seconds\\n\", end - start);

        // Timing tiled matrix multiplication
        int blockSize = 32; // Adjust block size as needed
        start = get_time();
        matrix_multiplication_tiling(M, N2, Result2, matrix_size, blockSize);
        end = get_time();
        printf(\"Tiled Matrix Multiplication Time: %.6f seconds\\n\", end - start);
...
app._unparsable_cell(
    ...
    // Plain vector addition without vectorization
    void vector_add_plain(float* x, float* y, float* z, int n, float scalar) {
        for (int i = 0; i < n; i++) {
            z[i] = x[i] * scalar + y[i];
        }
    }
    ...
    // Plain dot product without vectorization
    double dot_product_plain(const double* a, const double* b, int n) {
        double sum = 0.0;
        for (int i = 0; i < n; i++) {
            sum += a[i] * b[i];
        }
    ...
    double dot_product_intrinsics(const double* a, const double* b, int n) {
        ...
        int i;
        for (i = 0; i <= n - 4; i += 4) {
            __m256d a_vec = _mm256_load_pd(&a[i]);  // Load 4 elements from a
            __m256d b_vec = _mm256_load_pd(&b[i]);  // Load 4 elements from b
            __m256d prod_vec = _mm256_mul_pd(a_vec, b_vec); // Multiply a[i] and b[i]
            sum_vec = _mm256_add_pd(sum_vec, prod_vec);      // Add product to sum
        }
        double buffer[4];
        _mm256_store_pd(buffer, sum_vec);            // Store the result
    ...
    int main() {
        ...
        // Timing vector addition without vectorization (plain loop)
        double start = omp_get_wtime();
        vector_add_plain(x, y, z, N, scalar);
        double end = omp_get_wtime();
        printf(\"Plain Vector Addition Time: %.6f seconds\\n\", end - start);
        ...
        // Allocate arrays for dot product
        ...
        // Timing plain dot product
        start = omp_get_wtime();
        double dot_result_plain = dot_product_plain(a, b, N);
        end = omp_get_wtime();
        printf(\"Plain Dot Product Result: %.2f\\n\", dot_result_plain);
        printf(\"Plain Dot Product Time: %.6f seconds\\n\", end - start);

        // Timing dot product with AVX intrinsics
        start = omp_get_wtime();
        double dot_result_avx = dot_product_intrinsics(a, b, N);
        end = omp_get_wtime();
...
app._unparsable_cell(
    ...
    #define ACCESS_COUNT 10000000 // Number of accesses

    // Function without prefetching
    void process_without_prefetch(int* data, int* indices, int access_count) {
        volatile int sum = 0;
    ...
    // Function with prefetching
    void process_with_prefetch(int* data, int* indices, int access_count, int prefetch_distance) {
        volatile int sum = 0;
        for (int i = 0; i < access_count; i++) {
            // Prefetch data before processing
            if (i + prefetch_distance < access_count)
    ...
    int main() {
        int n = ARRAY_SIZE;
        int access_count = ACCESS_COUNT;
        int prefetch_distance = 64;  // Adjust as needed
        int* data = (int*)malloc(n * sizeof(int));
        int* indices = (int*)malloc(access_count * sizeof(int));
        ...
        // Initialize indices with random access pattern
        for (int i = 0; i < access_count; i++) {
            indices[i] = rand() % n;
        }
        ...
        start = get_time();
        process_without_prefetch(data, indices, access_count);
        end = get_time();
        printf(\"Processing without prefetching took %.6f seconds\\n\", end - start);

        // Process with prefetching
        start = get_time();
        process_with_prefetch(data, indices, access_count, prefetch_distance);
        end = get_time();
        printf(\"Processing with prefetching took %.6f seconds\\n\", end - start);

        // Output some of the data to verify correctness
        printf(\"First 10 elements of data array:\\n\");
        for (int i = 0; i < 10; i++) {
            printf(\"%d \", data[i]);
        }
        printf(\"\\n\");
...
app._unparsable_cell(
    r"""
    # Write the C code for in-place and out-of-place matrix transpose to a file
    c_code = \"\"\"
    #include <stdio.h>
    #include <stdlib.h>
    #include <time.h>

    // In-place matrix transpose function
    void transpose_inplace(float* matrix, int n) {
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
    ...
    // Out-of-place matrix transpose function
    void transpose_outofplace(float* src, float* dst, int n) {
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                dst[j * n + i] = src[i * n + j];
            }
    ...
    int main() {
        int n = 2000;  // Size of the matrix (n x n)
        float* matrix_inplace = (float*)malloc(n * n * sizeof(float));
        float* matrix_outofplace = (float*)malloc(n * n * sizeof(float));
        float* matrix_outofplace_result = (float*)malloc(n * n * sizeof(float));

        if (matrix_inplace == NULL || matrix_outofplace == NULL || matrix_outofplace_result == NULL) {
            printf(\"Memory allocation failed\\n\");
            return 1;
        }
        ...
        for (int i = 0; i < n * n; i++) {
            float value = (float)(i % 100);  // Modulo to keep values within a reasonable range
            matrix_inplace[i] = value;
            matrix_outofplace[i] = value;
        }

        clock_t start, end;
        double cpu_time_used;

        // In-place transpose timing
        start = clock();
        transpose_inplace(matrix_inplace, n);
        end = clock();
        cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
        printf(\"In-place transpose took %.6f seconds\\n\", cpu_time_used);

        // Out-of-place transpose timing
        start = clock();
        transpose_outofplace(matrix_outofplace, matrix_outofplace_result, n);
        end = clock();
        cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
        printf(\"Out-of-place transpose took %.6f seconds\\n\", cpu_time_used);

        // Verify that both transposes produce the same result
        int incorrect = 0;
        for (int i = 0; i < n * n; i++) {
            if (matrix_inplace[i] != matrix_outofplace_result[i]) {
                incorrect = 1;
                printf(\"Mismatch at index %d: in-place %f, out-of-place %f\\n\", i, matrix_inplace[i], matrix_outofplace_result[i]);
                break;
            }
        }
        if (!incorrect) {
            printf(\"Both transposes produce the same result.\\n\");
        }

        // Optional: Print a small portion of the transposed matrix
        /*
        printf(\"Sample of transposed matrix:\\n\");
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                printf(\"%.2f \", matrix_inplace[i * n + j]);
            }
            printf(\"\\n\");
        }
        */
        ...
        // Clean up
        free(matrix_inplace);
        free(matrix_outofplace);
        free(matrix_outofplace_result);
    ...
    # Save the C code to a file
    with open(\"transpose_compare.c\", \"w\") as file:
        file.write(c_code)

    # Compile the C program with gcc
    !gcc -o transpose_compare transpose_compare.c
...
app._unparsable_cell(
    r"""
    # Write the C code for speculative execution using likely() and unlikely() with detailed output
    c_code = \"\"\"
    #include <stdio.h>
    #include <stdlib.h>
    #include <time.h>
    ...
    // Function without speculative execution hints
    void process_without_likely(int* data, int size) {
    ...
    // Function with speculative execution hints
    void process_with_likely(int* data, int size) {
        volatile int count = 0;
        for (int i = 0; i < size; i++) {
            if (likely(data[i] >= 0)) {
                // Simulate computational work
    ...
    int main() {
        ...
        // Initialize data with a less predictable pattern
        for (int i = 0; i < size; i++) {
            if (i % 3 == 0) {
                data[i] = -i; // Approximately 33% negative numbers
                negative_count++;
            } else {
                data[i] = i;
                positive_count++;
        ...
        // Process without speculative execution hints
        start = clock();
        process_without_likely(data, size);
        end = clock();
        cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
        printf(\"Processing without likely() took %.6f seconds\\n\", cpu_time_used);

        // Process with speculative execution hints
        start = clock();
        process_with_likely(data, size);
        end = clock();
        cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
        printf(\"Processing with likely() took %.6f seconds\\n\", cpu_time_used);
...
app._unparsable_cell(
    ...
    // Function to simulate variable work
    void process(int i) {
        int thread_num = omp_get_thread_num();
        printf(\"Processing element %d by thread %d\\n\", i, thread_num);
        // Simulate variable computation time
        // Sleep for a time proportional to the element value
        usleep((i % 5) * 100000); // Sleep between 0 and 400 milliseconds
    }
