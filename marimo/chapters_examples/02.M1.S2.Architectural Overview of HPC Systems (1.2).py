...
def _(time):
    ...
    def fcfs_scheduler(jobs):
        ...
        for job in jobs:
            ...
            time.sleep(1)  # Simulate job execution with a short delay
            current_time += job[2]
...
def _(heapq, jobs, time):
    ...
    print("\n=== Priority-Based ===")
    priority_scheduler(jobs[:])  # Pass a copy of the jobs list to avoid modification
    return
...
def _(jobs, time):
    ...
    print("\n=== Round-Robin ===")
    round_robin_scheduler(jobs[:], time_slice=2)  # Pass a copy of the jobs list to avoid modification
    return
