...
def _(mo):
    mo.md(
        r"""
        # Parallel Quicksort using Divide and Conquer in Shared Memory (OpenMP)

        ### Explanation of Divide and Conquer in Shared Memory

        **Divide and Conquer** is an algorithmic technique where:
        - The problem is divided into smaller subproblems.
        - Each subproblem is solved independently (often recursively).
        - The solutions of the subproblems are combined to form the final solution.

        In shared memory systems like those using **OpenMP**, this process can be parallelized by assigning different threads to handle the subproblems concurrently.

        ### Steps of the Quicksort Algorithm:
        1. **Divide**: The array is partitioned into two smaller sub-arrays based on a pivot element.
            - Elements smaller than the pivot go to the left sub-array.
            - Elements larger than the pivot go to the right sub-array.
        2. **Conquer**: Each sub-array is sorted recursively using the same partitioning process.
        3. **Combine**: Once the sub-arrays are sorted, they are combined to form the final sorted array.

        In shared memory systems:
        - Each subproblem can be handled by different threads, allowing multiple parts of the array to be sorted concurrently.
        - **OpenMP** is used to create parallel sections or tasks, where each thread is assigned a different portion of the array to sort.

        ### C Code for Parallel Quicksort with OpenMP

        Below is the implementation of **Parallel Quicksort** using **OpenMP**. This code divides the work into sections that are handled by different threads, which run concurrently.
...
app._unparsable_cell(
    r"""
    # Install GCC with OpenMP support (if not already installed)
    !apt-get install gcc
    """,
    name="_"
)
...
app._unparsable_cell(
    r"""
    # This will create a C file named \"parallel_quicksort.c\" in Google Colab
    ...
    #define N 16  // Size of the array (can be increased for larger test cases)
    ...
    void parallel_quicksort(int arr[], int low, int high) {
        if (low < high) {
            int pi = partition(arr, low, high);  // Find pivot element

            // Create parallel tasks for recursive quicksort
            #pragma omp parallel sections
            {
                #pragma omp section
                {
                    printf(\"Thread %d sorting left half: [%d, %d]\\n\", omp_get_thread_num(), low, pi - 1);
                    parallel_quicksort(arr, low, pi - 1);  // Sort elements before partition
                }
                #pragma omp section
                {
                    printf(\"Thread %d sorting right half: [%d, %d]\\n\", omp_get_thread_num(), pi + 1, high);
                    parallel_quicksort(arr, pi + 1, high);  // Sort elements after partition
                }
            }
        }
    }
    ...
    int main() {
        ...
        #pragma omp parallel
        {
            #pragma omp single
            {
                printf(\"Starting parallel quicksort with OpenMP...\\n\");
                parallel_quicksort(arr, 0, N - 1);  // Sort the entire array
            }
        }
    ...
    # Compile the C file with OpenMP support
    !gcc -fopenmp parallel_quicksort.c -o parallel_quicksort
