...
def _(Process, dist, os, torch):
    ...
    def spawn_processes_1():
        ...
        processes = []
        for rank in range(size):
            p = Process(target=init_process_1, args=(rank, size, train))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
...
def _(nn, optim, torch):
    ...
    def train_1(model, loader, criterion, optimizer, device):
        model.train()
    ...
    train_1(model_3, train_loader, criterion, optimizer, device)
    return DataLoader, SimpleNN, torchvision, transforms
...
@app.cell
def _(DataLoader, SimpleNN, nn, optim, torch, torchvision, transforms):
    ...
    def train_2(model, loader, criterion, optimizer, device):
        model.train()
        for epoch in range(5):
            running_loss = 0.0
            for images, labels in loader:
    ...
    def main():
        ...
        train_2(model, train_loader, criterion, optimizer, device)
...
def _(
    ...
    def main_1():
        ...
        mp.spawn(train_3, args=(world_size,), nprocs=world_size, join=True)
    if __name__ == '__main__':
        main_1()
    return DataLoader, DistributedSampler, SimpleNN_2, mp
...
def _(
    ...
    def train_4(rank, world_size, master_addr, master_port):
    ...
    def main_2():
        ...
        master_port = '12355'
        mp.spawn(train_4, args=(world_size, master_addr, master_port), nprocs=torch.cuda.device_count(), join=True)
    if __name__ == '__main__':
        num_nodes = 2
        main_2()
    return
...
def _(Process, dist, nn, optim, os, torch):
    ...
    def spawn_processes_2():
        ...
        processes = []
        for rank in range(size):
            p = Process(target=init_process_2, args=(rank, size, train_5))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
...
@app.cell
def _(Process, dist, os, torch):
    def init_process_3(rank, size, fn, backend='gloo'):
        os.environ['MASTER_ADDR'] = 'localhost'
        os.environ['MASTER_PORT'] = '12355'
        dist.init_process_group(backend, rank=rank, world_size=size)
        fn(rank, size)
    ...
    def spawn_processes_3():
        size = 2
        processes = []
        for rank in range(size):
            p = Process(target=init_process_3, args=(rank, size, run_2))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
